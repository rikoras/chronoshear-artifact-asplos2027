...
class MakeCondPart(sg: StatementGraph, rn: Renamer, extIOtypes: Map[String, Type]) extends LazyLogging {
  ...
  def getPartOutputsToDeclare(): Seq[(String,Type)] = {
    ...
    // A top-level output may be read back by another conditional partition
    // (PTW's perf.l2hit is one concrete example).  It is still a part output
    // for dependency propagation, but the module port declaration already
    // owns its C++ struct member.  Emitting it again here produces an invalid
    // duplicate field.  Keep it in CondPart.outputsToDeclare and suppress only
    // the redundant top-level declaration.
    allPartOutputTypes.filterNot { case (name, _) =>
      extIOtypes.contains(name)
    }.toSeq
  }
