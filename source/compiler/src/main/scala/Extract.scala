...
object Extract extends LazyLogging {
  ...
  def findDependencesExpr(e: Expression): Seq[String] = {
    val result = e match {
      ...
      case sb: SubField =>
        val innerResult = findDependencesExpr(sb.expr)
        if (innerResult.isEmpty) Seq()
        else Seq(s"${innerResult.head}.${sb.name}")
      case w: WSubAccess => Seq(w.expr, w.index) flatMap findDependencesExpr
      case si: SubIndex =>
        val innerResult = findDependencesExpr(si.expr)
        if (innerResult.isEmpty) Seq()
        else Seq(s"${innerResult.head}[${si.value}]")
      case sa: SubAccess => Seq(sa.expr, sa.index) flatMap findDependencesExpr
      case p: DoPrim => p.args flatMap findDependencesExpr
      case v: ValidIf => Seq(v.cond, v.value) flatMap findDependencesExpr
  ...
  def findDependencesStmt(s: Statement): Seq[HyperedgeDep] = s match {
    ...
    case ru: RegUpdate => {
      if(ru.info.isInstanceOf[FileInfo] && ru.info.asInstanceOf[FileInfo].escaped == "final_commit") {
        Seq(HyperedgeDep(emitExpr(ru.regRef)+"$final_commit", findDependencesExpr(ru.expr), s))
      }
      else{
        Seq(HyperedgeDep(emitExpr(ru.regRef)+"$final", findDependencesExpr(ru.expr), s))
      }
    }
    case cru: CondRegUpdate => {
      // Dependencies: cond + expr only. regRef is the WRITE target, NOT a read dependency.
      val deps = (findDependencesExpr(cru.cond) ++ findDependencesExpr(cru.expr)).distinct
      val tag = if (cru.info.isInstanceOf[FileInfo] && cru.info.asInstanceOf[FileInfo].escaped == "final_commit")
        "$final_commit" else "$final"
      Seq(HyperedgeDep(emitExpr(cru.regRef) + tag, deps, s))
    }
    ...
    case lv: LBNVerify =>
      val regName = emitExpr(lv.regRef)
      val oracleDeps = findDependencesExpr(lv.oracleRef)
      val deps = Seq(regName + "$final_commit") ++ oracleDeps
      Seq(HyperedgeDep("VERIFY_" + regName, deps.distinct, lv))
    ...
    case lt: LookupTable =>
      Seq(HyperedgeDep(lt.name, findDependencesExpr(lt.indexExpr), lt))
    case EmptyStmt => Seq()
  ...
  def flattenWholeDesign(circuit: Circuit, squishOutConnects: Boolean, isSIMD: Boolean = false): Seq[Statement] = {
    val allInstances = findAllModuleInstances(circuit)
    ...
    if (squishOutConnects) {
      ...
      squishOutPassThroughConnects(allBodiesFlattened, namesToExclude, isSIMD)
  ...
  def squishOutPassThroughConnects(bodies: Seq[Statement],
                                   namesToExclude: Set[String], isSIMD:Boolean): Seq[Statement] = {
    ...
    val chainRenames = findChainRenames(StatementGraph(straightConnects, isSIMD))
