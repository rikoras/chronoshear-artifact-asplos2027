...
object RegFromMem1 extends Pass {
  ...
  // NoClockConnects remains explicitly ordered before this pass in the
  // legacy pipeline.  SIMD-v2 deliberately retains Clock connects until its
  // hierarchy-aware ClockDomainLowering has converted derived clocks to
  // data enables, so do not pull NoClockConnects back in transitively.
