...
//          println(s"${m.name} - $leakedNodesFound.keys")
      ...
      // Empty modules are legal after early cleanup (for example a clock-only
      // wrapper whose data semantics are handled by the v2 clock lowering).
      // This legacy compatibility pass has nothing to rewrite in them.
      case _ => m
    }
