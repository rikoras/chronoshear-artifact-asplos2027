...
#ifndef CHISA_BASELINE_THREADS
#error "Specify the compiled baseline thread count"
#endif
#include "verilated.h"
#if VM_TRACE
#include <memory>
...
static std::atomic<bool> baseline_program_ready{false};
class BaselineDtm final : public dtm_t {
 public:
  using dtm_t::dtm_t;
  void start() override {
    htif_t::start();  // load_program() followed by the actual DTM reset/resume
    baseline_program_ready.store(true, std::memory_order_release);
  }
};
...
{
  unsigned random_seed = 1;  // deterministic benchmark default
...
done_processing:
  ...
  Verilated::threadContextp()->threads(CHISA_BASELINE_THREADS);
  Verilated::randSeed(random_seed);
  ...
  dtm = new BaselineDtm(htif_argc, htif_argv);
  ...
  // The private benchmark timer stops at an exclusive upper bound while the
  // RTL continues to normal program completion. Reset/loading are untimed.
  const uint64_t timing_from = getenv("VERILATOR_TIMING_FROM")
      ? strtoull(getenv("VERILATOR_TIMING_FROM"), NULL, 10) : ~0ULL;
  const uint64_t timing_until = getenv("VERILATOR_TIMING_UNTIL")
      ? strtoull(getenv("VERILATOR_TIMING_UNTIL"), NULL, 10) : max_cycles;
  if (timing_from != ~0ULL && (timing_from >= timing_until || timing_until > max_cycles)) {
    fprintf(stderr, "invalid benchmark timing interval\\n");
    return 1;
  }
  bool timing_on = false, timing_finished = false;
  std::chrono::steady_clock::time_point timing_t0, timing_t1;

  uint64_t ready_cycle = ~0ULL, first_run_cycle = ~0ULL, first_run_pc = 0;
  uint64_t post_start_commits = 0, roi_run_commits = 0, roi_debug_commits = 0;
  while (trace_count < max_cycles) {
    if (ready_cycle == ~0ULL && baseline_program_ready.load(std::memory_order_acquire))
      ready_cycle = trace_count;
    if (trace_count == timing_from) { paper_perf_control(true); timing_t0 = std::chrono::steady_clock::now(); timing_on = true; }
...
#endif
    trace_count++;
    if (timing_on && trace_count == timing_until) {
      timing_t1 = std::chrono::steady_clock::now();
      timing_finished = true;
      paper_perf_finish(timing_until-timing_from);
    }
  }
  if (timing_finished) {
    double ns = std::chrono::duration<double, std::nano>(timing_t1 - timing_t0).count();
    uint64_t timed = timing_until - timing_from;
    fprintf(stderr, "TIMED timed_cycles=%lu ns_per_cycle=%.1f timed_seconds=%.3f\n", (unsigned long)timed, ns / (double)timed, ns * 1e-9);
  }
  ...
  if (timing_from != ~0ULL && !timing_finished) {
    fprintf(stderr, "benchmark interval was not fully executed\\n");
    if (!ret) ret = 3;
  }
  const bool after_loading = ready_cycle != ~0ULL && ready_cycle < timing_from;
  fprintf(stderr, "BASELINE_ALIGNMENT context_threads=%u program_ready_cycle=%llu first_run_cycle=%llu first_run_pc=%llx post_start_commits=%llu roi_run_commits=%llu roi_debug_commits=%llu after_loading=%d\n",
      Verilated::threadContextp()->threads(), (unsigned long long)ready_cycle,
      (unsigned long long)first_run_cycle, (unsigned long long)first_run_pc,
      (unsigned long long)post_start_commits, (unsigned long long)roi_run_commits,
      (unsigned long long)roi_debug_commits, int(after_loading));
  if (timing_from != ~0ULL && !after_loading && !ret) ret = 3;
  if (dtm) delete dtm;
  if (jtag) delete jtag;
  if (tile) delete tile;
