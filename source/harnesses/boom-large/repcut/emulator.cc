...
bool verbose;
bool done_reset;
static std::atomic<bool> program_ready{false};
class MarkedDtm final : public dtm_t {
 public:
  using dtm_t::dtm_t;
  void start() override {
    htif_t::start();
    program_ready.store(true, std::memory_order_release);
  }
};
...
done_processing:
  ...
  dtm = new MarkedDtm(htif_argc, htif_argv);
  ...
  const uint64_t begin = getenv("VERILATOR_TIMING_FROM") ? strtoull(getenv("VERILATOR_TIMING_FROM"), nullptr, 10) : ~0ULL;
  const uint64_t end = getenv("VERILATOR_TIMING_UNTIL") ? strtoull(getenv("VERILATOR_TIMING_UNTIL"), nullptr, 10) : max_cycles;
  uint64_t ready_cycle = ~0ULL;
  bool timing_started=false, timing_finished=false;
  std::chrono::steady_clock::time_point t0,t1;
  if (begin != ~0ULL && (begin >= end || end > max_cycles)) return 1;
  while (trace_count < max_cycles) {
    if (ready_cycle == ~0ULL && program_ready.load(std::memory_order_acquire)) ready_cycle = trace_count;
    if (trace_count == begin) { paper_perf_control(true); t0=std::chrono::steady_clock::now(); timing_started=true; }
    ...
    trace_count++;
    if (timing_started && trace_count == end) {
      t1=std::chrono::steady_clock::now(); timing_finished=true;
      paper_perf_finish(end-begin);
    }
  }
  ...
  if (dtm->exit_code()) {
    ...
    fprintf(stderr, "*** PASSED *** Completed after %" PRIu64 " cycles\n", trace_count);
  }

  if (timing_finished) {
    const double ns=std::chrono::duration<double,std::nano>(t1-t0).count();
    fprintf(stderr,"TIMED timed_cycles=%llu ns_per_cycle=%.3f timed_seconds=%.6f\n",
            (unsigned long long)(end-begin),ns/double(end-begin),ns*1e-9);
  }
  const bool aligned=ready_cycle != ~0ULL && ready_cycle < begin;
  fprintf(stderr,"BASELINE_ALIGNMENT program_ready_cycle=%llu after_loading=%d\n",
          (unsigned long long)ready_cycle,int(aligned));
  if (begin != ~0ULL && (!aligned || !timing_finished) && !ret) ret=3;
  if (dtm) delete dtm;
  if (tile) delete tile;
