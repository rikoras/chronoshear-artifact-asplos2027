...
done_processing:
  ...
  auto roi_t0=std::chrono::steady_clock::now(), roi_t1=roi_t0;
...
#endif
    trace_count++;
    if (trace_count==803200) roi_t0=std::chrono::steady_clock::now();
    if (trace_count==1000000) roi_t1=std::chrono::steady_clock::now();
  }
  ...
  if (trace_count >= 1000000) std::fprintf(stderr,"TIMED timed_cycles=196800 ns_per_cycle=%.3f\n", std::chrono::duration<double,std::nano>(roi_t1-roi_t0).count()/196800.0);
  if (dtm) delete dtm;
  if (jtag) delete jtag;
  if (tile) delete tile;
