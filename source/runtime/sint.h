...
public:
  ...
  SInt<cmax(w_ - shamt,1)> shr() const {
    ...
    SInt<cmax(w_ - shamt,1)> result = [&]() {
      if constexpr (shamt >= w_)
        return SInt<cmax(w_ - shamt,1)>(ui.template bits<w_-1,w_-1>());
      else
        return SInt<cmax(w_ - shamt,1)>(ui.template core_bits<w_-1, shamt>());
    }();
...
private:
  ...
  void sign_extend(int sign_index = (w_-1)) {
    ...
    // Keep the register representation exact without shifting a negative
    // signed value (undefined in C++17). The mask also covers bit 63.
    const uint64_t low_mask = UINT64_MAX >> (63 - sign_offset);
    ui.words_[sign_word] = (ui.words_[sign_word] & low_mask) |
                          (is_neg ? ~low_mask : UINT64_C(0));
