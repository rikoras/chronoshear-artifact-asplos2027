...
public:
  ...
  UInt<w_ + other_w> cat(const UInt<other_w> &other) const {
    if constexpr (w_ + other_w <= 64 && n_ == 1 && UInt<other_w>::NW == 1) {
      return UInt<w_ + other_w>((static_cast<uint64_t>(words_[0]) << other_w) |
                                 static_cast<uint64_t>(other.words_[0]));
    } else {
  ...
  UInt<cmax(w_, other_w) + 1> operator+(const UInt<other_w> &other) const {
    ...
    if constexpr (result_w <= 64 && n_ == 1 && UInt<other_w>::NW == 1) {
      return UInt<result_w>(static_cast<uint64_t>(words_[0]) +
                            static_cast<uint64_t>(other.words_[0]));
    } else {
  ...
  UInt<w_ + other_w> operator*(const UInt<other_w> &other) const {
    ...
    if constexpr (w_ <= 64 && other_w <= 64) {
      // A single native product retains all 128 result bits. Avoid four
      // 32-bit partial products and their explicit carry propagation for
      // the scalar datapath's usual one-word operands.
      const unsigned __int128 product =
          static_cast<unsigned __int128>(static_cast<uint64_t>(words_[0])) *
          static_cast<uint64_t>(other.words_[0]);
      result.words_[0] = static_cast<uint64_t>(product);
      if constexpr (w_ + other_w > 64)
        result.words_[1] = static_cast<uint64_t>(product >> 64);
      result.mask_top_unused();
      return result;
    }
    uint64_t carry = 0;
  ...
  template<int other_w>
  UInt<cmax(w_, other_w)> operator&(const UInt<other_w> &other) const {
    constexpr int rw = cmax(w_, other_w);
    if constexpr (w_ == other_w) {
      ...
      UInt<rw> a = pad<rw>();
      UInt<rw> b = other.template pad<rw>();
      UInt<rw> result;
      for (int i = 0; i < UInt<rw>::NW; i++)
        result.words_[i] = a.words_[i] & b.words_[i];
      return result;
    }
  }
  ...
  UInt<1> xorr() const {
    uint64_t result = 0;
    for (int i = 0; i < n_; i++) {
      uint64_t v = static_cast<uint64_t>(words_[i]);
      v ^= (v >> 1);
      v ^= (v >> 2);
      v ^= (v >> 4);
      v ^= (v >> 8);
  ...
  UInt<w_> operator>>(const UInt<other_w> &other) const {
    ...
    for (uint64_t i=word_down; i < n_; i++) {
      result.words_[i - word_down] = static_cast<uint64_t>(words_[i]) >> bits_down;
      if ((bits_down != 0) && (i < n_-1))
        result.words_[i - word_down] |= static_cast<uint64_t>(words_[i + 1]) << cap(kWordSize - bits_down);
    }
    return result;
  }
  ...
  UInt<1> operator<=(const UInt<w_> &other) const {
    if constexpr (n_ == 1) {
      return UInt<1>(words_[0] <= other.words_[0]);
    } else {
  ...
  char* write_char_buf(char* buffer) const {                                  
      ...
      for (int i = n_-1; i >= 0; i--) {
          int start_point = i == n_-1 ? bits_in_top_word_-1 : kWordSize-1;
          for (int j = start_point; j >= 0; j--) {
              buffer[idx++] = (words_[i] & (1ULL << j)) ? '1' : '0';
          }   
      }   
...
private:
  ...
  static constexpr word_t kTopMask =
      (bits_in_top_word_ == WW) ? ~word_t(0) : (word_t(1) << bits_in_top_word_) - word_t(1);
  ...
  void mask_top_unused() {
    if constexpr (bits_in_top_word_ != WW) {
      words_[n_-1] &= kTopMask;
    }
  }
  ...
  UInt<hi - lo + 1> core_bits() const {
    ...
    if constexpr (rw == 0) {
      return UInt<rw>();
    } else if constexpr (n_ == 1 && UInt<rw>::NW == 1) {
      if constexpr (lo == 0)
        return UInt<rw>(static_cast<uint64_t>(words_[0]));
      else
        return UInt<rw>(static_cast<uint64_t>(words_[0]) >> lo);
    } else {
      UInt<rw> result;
  ...
  void print_to_stream(std::ostream& os) const {
    ...
    uint64_t top_word_mask = bits_in_top_word_ == kWordSize ? ~uint64_t(0) :
                               (1ULL << cap(bits_in_top_word_)) - 1ULL;
