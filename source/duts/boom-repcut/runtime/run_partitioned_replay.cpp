// Two-level parallelism replay: one host thread per emitted partition, each
...
void run_partition(int p, const Options& options, const Trace& trace,
  ...
  try {
    ...
    for (std::uint64_t first = 0; first < trace.header.cycles; first += kReplayW) {
      ...
      dut->eval_partition(p, forward, true, options.verbose, !reset);
...
int main(int argc, char** argv) {
  try {
    ...
    if (partitions <= 0) throw std::runtime_error("header was emitted without partitions");
    const int threads = options.threads < 0 ? partitions : options.threads;
    if (threads != partitions)
      throw std::runtime_error("--threads must equal the emitted partition count (" +
    ...
    std::printf("%s threads=%d cycles=%" PRIu64 " max_ns_per_cycle=%.1f sum_ns_per_cycle=%.1f wall_seconds=%.3f\n",
                ok ? "PARTITIONED_REPLAY_OK" : "PARTITIONED_REPLAY_FAILED", threads, trace.header.cycles,
